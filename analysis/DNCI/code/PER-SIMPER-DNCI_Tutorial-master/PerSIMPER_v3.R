...
PerSIMPER <- function(matrixSIMP,
                      ...
                      Nperm=1000,
                      plotSIMPER = TRUE,
                      time_sparse = 180){    # add the possibility to change the number of permutations
  ...
  for (i in 1:Nperm)  {
    T_e <- Sys.time() 
    if(as.numeric(T_e - T_s, units = "secs") > time_sparse) return("too sparse") 
    ...
    repeat {
      ...
      for(j in 1:length(dp4$perm[[1]][,2])) {
        if(sum(dp4$perm[[1]][j,]) == 0){v <- FALSE} # check no zero row sum
      }
      t_e <- Sys.time() 
      if(as.numeric(t_e - t_s, units="secs") > 180) return("too sparse") # calculation time longer than 3 mins
      if(v == TRUE) break
    }
