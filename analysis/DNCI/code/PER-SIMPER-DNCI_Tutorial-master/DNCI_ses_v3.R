...
DNCI.ses <- function(x, grouping,id = "no_name", Nperm = 99, count = TRUE, plotSIMPER = FALSE, dataTYPE = "count", time_sparse = 180) { #this calculates the metric using PERSIMPER - now the output included DELTAd-n, sd of DELTA.d-n and confidence interval
  groups <- sort(unique(grouping))
  stopifnot(length(groups) == 2)
  results = PerSIMPER(x, grouping,  count = count, Nperm = Nperm, plotSIMPER = plotSIMPER, dataTYPE = dataTYPE, time_sparse = time_sparse)
  
  if(identical(results, "too sparse")) return("too sparse")
