...
# reconstruct random forest
n_rand <- 99
Accuracy[1, ] <- RF_t4_all$performance
PredMat_i[ ,2] <- predict(RF_t4_all$model$i, newdata = dat_test)$ypred
...
sd(Accuracy$i) # 0.0003401895
sd(Accuracy$j) # 0.0003501511
sd(Accuracy$k) # 0.0004861283

colnames(Accuracy) <- c("Dispersal", "Abiotic", "Biotic")
Accuracy %>%
  gather(key = "Process", value = "Accuracy", Dispersal:Biotic) %>%
